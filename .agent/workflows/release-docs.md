...
You are a documentation generator for the compound-engineering plugin. Your job is to ensure the documentation site at `.agent/skills/compound-engineering/docs/` is always up-to-date with the actual plugin components.
...
```bash
# Count agents
ls .agent/skills/compound-engineering/agents/*.md | wc -l

# Count commands
ls .agent/skills/compound-engineering/commands/*.md | wc -l

# Count skills
ls -d .agent/skills/compound-engineering/skills/*/ 2>/dev/null | wc -l

# Count MCP servers
ls -d .agent/skills/compound-engineering/mcp-servers/*/ 2>/dev/null | wc -l
```

Read all component files to get their metadata:

### Agents
For each agent file in `.agent/skills/compound-engineering/agents/*.md`:
...
### Commands
For each command file in `.agent/skills/compound-engineering/commands/*.md`:
- Extract the frontmatter (name, description, argument-hint)
- Categorize as Workflow or Utility command

### Skills
For each skill directory in `.agent/skills/compound-engineering/skills/*/`:
- Read the SKILL.md file for frontmatter (name, description)
- Note any scripts or supporting files

### MCP Servers
For each MCP server in `.agent/skills/compound-engineering/mcp-servers/*/`:
...
1. **`.agent/skills/compound-engineering/.claude-plugin/plugin.json`**
...
3. **`.agent/skills/compound-engineering/README.md`**
...
cat .agent/skills/compound-engineering/.claude-plugin/plugin.json | jq .

# Verify counts match
echo "Agents in files: $(ls .agent/skills/compound-engineering/agents/*.md | wc -l)"
grep -o "[0-9]* specialized agents" .agent/skills/compound-engineering/docs/index.html

echo "Commands in files: $(ls .agent/skills/compound-engineering/commands/*.md | wc -l)"
grep -o "[0-9]* slash commands" .agent/skills/compound-engineering/docs/index.html
```
