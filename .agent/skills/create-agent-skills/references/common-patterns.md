...
<flexible_guidance>
Use when Antigravity should adapt the format based on context:
...
<principle>
Choose one term and use it throughout the skill. Inconsistent terminology confuses Antigravity and reduces execution quality.
</principle>
...
Antigravity must now interpret: Are "API routes" and "URLs" the same? Are "fields", "boxes", "elements", and "controls" the same?
</bad_example>
...
Antigravity must now research and compare all options before starting. This wastes tokens and time.
</bad_example>
...
**Why it matters**: Vague descriptions prevent Antigravity from discovering and using the skill appropriately.
</pitfall>
...
**Why it matters**: Antigravity may only partially read deeply nested files. Keep references one level deep from SKILL.md.
</pitfall>
...
<description>
Keep SKILL.md concise by linking to detailed reference files. Antigravity loads reference files only when needed.
</description>
...
- Antigravity only reads relevant reference files
...
- Antigravity can fix issues without guessing
...
<checklist_pattern>
<description>
For complex multi-step workflows, provide a checklist Antigravity can copy and track progress.
</description>
